---
title: "Configure the Binance SDK Clients"
sidebarTitle: "Configuration"
description: "Learn every configuration option for REST and WebSocket clients in the Binance SDK, including timeouts, proxy, logging, beautification, and custom base URLs."
---
...
<ParamField path="disableTimeSync" type="boolean" default="true">
  When `false`, the SDK automatically fetches the Binance server time and computes a clock drift offset, then applies that offset to every signed request timestamp. Set this to `false` if your system clock is unreliable or if you're running in an environment where small clock skew regularly triggers timestamp errors. The default is `true` (sync disabled) to avoid the extra round-trip on startup.
</ParamField>
