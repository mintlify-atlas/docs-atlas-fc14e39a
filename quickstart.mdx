---
title: "Get Started with the Binance SDK"
sidebarTitle: "Quickstart"
description: "Install the binance npm package, create your first REST client, and make a live API call to Binance in under 5 minutes."
---
...
<Tabs>
  <Tab title="TypeScript">
    ...
    // Subscribe to Spot market topics (route to 'main')
    wsClient.subscribe(
  ...
  <Tab title="JavaScript">
    ...
    // Subscribe Spot topics to the 'main' connection family
    wsClient.subscribe(
      ['btcusdt@trade', 'btcusdt@bookTicker', 'btcusdt@depth10@100ms'],
      'main',
    );
...
The second argument to `subscribe()` is a `WS_KEY_MAP` value that routes the connection to the correct Binance endpoint family. Use `'main'` for Spot and Margin, `'usdm'` / `'usdmMarket'` for USD-M Futures, and `'coinm'` for COIN-M Futures.
